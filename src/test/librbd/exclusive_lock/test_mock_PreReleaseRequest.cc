...
public:
  ...
  void expect_detect_image_cache(MockTestImageCtx &mock_image_ctx, bool exists) {
    EXPECT_CALL(*mock_image_ctx.io_image_dispatcher,
	        dispatch_exist(io::IMAGE_DISPATCH_LAYER_WRITEBACK_CACHE))
...
TEST_F(TestMockExclusiveLockPreReleaseRequest, Success) {
  ...
  expect_detect_image_cache(mock_image_ctx, true);
...
TEST_F(TestMockExclusiveLockPreReleaseRequest, SuccessJournalDisabled) {
  ...
  expect_detect_image_cache(mock_image_ctx, false);
