...
public:
  ...
  void expect_start_hook_points(MockTestImageCtx &mock_image_ctx, int r) {
    EXPECT_CALL(*mock_image_ctx.plugin_registry, start_hook(_))
                  .WillOnce(CompleteContext(r, mock_image_ctx.image_ctx->op_work_queue));
  }
...
TEST_F(TestMockExclusiveLockPostAcquireRequest, Success) {
  ...
  expect_start_hook_points(mock_image_ctx, 0);
