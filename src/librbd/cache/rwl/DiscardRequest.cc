...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::cache::rwl:DiscardRequest: " \
                           << this << " " << __func__ << ": "
...
void DiscardRequest<I>::delete_image_cache_file() {
  ...
  if (m_cache_state->present &&
      ...
      std::filesystem::exists(m_cache_state->path)) {
    std::filesystem::remove(m_cache_state->path);
  }
...
template <typename I>
void DiscardRequest<I>::send_remove_image_cache_state() {
  CephContext *cct = m_image_ctx.cct;
  ldout(cct, 10) << dendl;
...
template class librbd::cache::rwl::DiscardRequest<librbd::ImageCtx>;
