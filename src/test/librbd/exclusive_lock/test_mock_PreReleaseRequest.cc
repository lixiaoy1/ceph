...
public:
  ...
  void expect_close_hook_points(MockTestImageCtx &mock_image_ctx, int r) {
    EXPECT_CALL(*mock_image_ctx.plugin_registry, shutdown_hook(_))
                  .WillOnce(CompleteContext(r, mock_image_ctx.image_ctx->op_work_queue));
  }
...
TEST_F(TestMockExclusiveLockPreReleaseRequest, Success) {
  ...
  expect_close_hook_points(mock_image_ctx, 0);
